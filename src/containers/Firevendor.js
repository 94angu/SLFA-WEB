...
*/
class Firevendor extends Component {
  ...
  */
  ...
  findFirebasePath(){
      ...
      var firebasePath=(this.props.route.path.replace("/firevendor/","").replace(":sub",""))+(this.props.params&&this.props.params.sub?this.props.params.sub:"").replace(/\+/g,"/");
  ...
  */
  ...
  deleteFieldAction(key,isItArrayItem=false){
    ...
    var firebasePathToDelete=(this.props.route.path.replace("/firevendor/","").replace(":sub",""))+(this.props.params&&this.props.params.sub?this.props.params.sub:"").replace(/\+/g,"/");
    if(key!=null){
                ...
                {this.state.elements&&this.state.elements.length>0?(
                  <CardUI  name={"elements"} title={this.state.lastPathItem+"' elements"}  showAction={true} action={()=>this.refs.simpleDialog.show()}>
                    {this.state.elements?this.state.elements.map((item)=>{
                                  var theLink="/firevendor/"+this.state.completePath+Config.adminConfig.urlSeparator+item.uidOfFirebase;
